...
scena <- rbind(scena[1, ], scena[13:14, ])
# scena <- scena[13, ]
...
for (sc in 1:nrow(scena)){ # For each combination of scenario
  ...
  for (g in 1:nrow(grad)){ # For each step in the gradient
    ...
    for (s in 1:(nb.sim)){ # For each simulation
      ...
      #         file = paste0("Roedeer", nYears, "_", nb.sim, "-",
      #                       "yd_", yd, "-ar_", ar, ".rds"))
...
# --- Plot lynx predation relaxing


ggplot(Roerate[which(Roerate$yd == 0), ], aes(x=ar, y=ext25*100)) +
  geom_bar(stat = "identity") +
  # theme(legend.key.height= unit(5, 'cm'),
  #       text = element_text(size = 50),
  #       axis.text = element_text(size = 50)) +
  ylim(0, 100) +
  labs(title = "Populations at quasi-extinction
       - Scenario: with only lynx predation", x = "Time (Years)", y = "Proportion (%)")
